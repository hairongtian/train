...
<template>
  <v-touch @swipeleft="next" @swiperight="prev" @swipeup="setWeekMonth('week')" @swipedown="setWeekMonth('month')">
    <div class="calendar">
        <div class="calendar-tools">
            ...
            <div class="calendar-info">
                ...
                <div class="month">
                    <div>
                        <span>{{year}}.{{zeroPad(month+1)}}.{{day}}</span>
                        <span>周二</span>
                        <i  class="todayIcon" @click="setToday()"></i>
                    </div>
                </div>
            </div>
        </div>
        ...
        <tbody v-if="weekDisplay">
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-if="k1==currentWeek" v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
                <span class="today" v-if="year==new Date().getFullYear()&& month==new Date().getMonth()&&child.day==new Date().getDate()">今</span>
                <span v-else :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}">{{child.day}}</span>
...
export default {
    ...
    methods: {
          ...
          //e.stopPropagation()
            if(this.weekDisplay){
              ...
              if(this.currentWeek >=0 ){
                ...
                this.render(this.year, this.month)
                if(this.fillDays(this.days[5]).includes(1)&&this.days[5][0].day>this.days[5][6].day){
                  this.currentWeek = 5
                }else{
                  this.currentWeek = 4
                }
        ...
        next(e) {
            //e.stopPropagation()
            if(this.weekDisplay){
              if((this.days[this.currentWeek][6].day<this.days[this.currentWeek][0].day && this.currentWeek!=0)|| this.days[this.currentWeek+1][0].day<this.days[this.currentWeek][6].day){
                if (this.month == 11) {
                    this.month = 0
                    this.year = parseInt(this.year) + 1
                } else {
